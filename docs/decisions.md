...
**2026-07-10 실측**: `docker exec omnigent-ollama ollama pull gpt-oss:120b-cloud` → `success`(OLLAMA_API_KEY 인증 정상). `/v1/hosts` 의 `configured_harnesses` 가 `opencode: true`, `claude-native: true`, `codex: "needs-auth"`(의도대로 보류 상태) 를 정확히 보고 — 커스텀 이미지·구독 인증·사이드카 설계 전부 실증됨.
